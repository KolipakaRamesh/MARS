...
> [!NOTE]
> You might notice that we run dependency installation commands (`pip install`, `npm install`) frequently. This is to ensure the local environment is always in sync with the project's requirements, especially after updates. If the packages are already installed, these commands run very quickly as a check.
...
| Metric | Meaning |
|---|---|
| **Prompt Tokens** | The number of tokens sent to the LLM (system prompt + user message + history). |
| **Completion Tokens** | The number of tokens generated by the LLM in its response. |
| **Total Tokens** | The sum of prompt and completion tokens. This directly correlates with cost. |
| **Latency (ms)** | The wall-clock time taken for the LLM to process the request and return a response, measured in milliseconds. |
